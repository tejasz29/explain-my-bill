  ...
  return (
    <>
      ...
      <button
        ...
        onDrop={onDrop}
        className={`group glass-panel soft-grid relative w-full overflow-hidden rounded-2xl border p-6 transition-all duration-200 md:p-8 lg:p-10 ${
          disabled
            ? "cursor-wait opacity-70"
            : "cursor-pointer hover:-translate-y-0.5 hover:shadow-xl"
        } ${isDragging ? "border-amber-500 bg-amber-50/80" : "border-slate-200 bg-white"}`}
      ...
      >
        ...
        <div className="relative z-10 grid gap-8 xl:grid-cols-[1.2fr_0.8fr] xl:items-end">
          <div className="max-w-2xl space-y-5">
            <span className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white/85 px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-slate-600">
            ...
            <div>
              <h2
                className="text-3xl font-bold tracking-tight text-slate-950 md:text-5xl"
            ...
            <div className="flex flex-wrap gap-3 text-sm text-slate-600">
              <span className="inline-flex items-center gap-2 rounded-full bg-white/80 px-4 py-2 border border-slate-200">
          ...
          <div className="rounded-2xl border border-white/70 bg-white/72 p-5 shadow-sm backdrop-blur">
            ...
            <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3 text-sm text-slate-600">
              {selectedFileName ? (
                <>
                  Ready to analyze:{" "}
                  <span className="font-semibold text-slate-950">{selectedFileName}</span>
                </>
              ) : (
